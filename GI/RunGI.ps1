...
try {
    Register-ScheduledTask -TaskName "GI" -InputObject $task -ErrorAction Stop
    Write-Host "Scheduled task 'GI' created successfully."
} catch {
    $errorMessages += "Error registering the scheduled task: $_"
}

# Create a shortcut to run the scheduled task
$shortcutPath = [System.IO.Path]::Combine([System.Environment]::GetFolderPath('Desktop'), "Run GI.lnk")
$shortcut = (New-Object -ComObject WScript.Shell).CreateShortcut($shortcutPath)

# Set the shortcut properties
$shortcut.TargetPath = "C:\Windows\System32\schtasks.exe"
$shortcut.Arguments = "/run /TN GI"
$shortcut.WorkingDirectory = "C:\Windows\System32"
$shortcut.IconLocation = $exePath  # Set the icon to the selected executable
$shortcut.Save()

Write-Host "Shortcut 'Run GI' created on the Desktop."
